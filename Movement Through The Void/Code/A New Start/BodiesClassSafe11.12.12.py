...
class Body(object):
    def __init__(self, velocity = vector(0,0,0), mass = 0, position = vector(0,0,0), identity = "unknown"):
        ...
        ##Needs work to determine size
        self.radius = log10(mass)+.1
...
##print GetSystem()
x = Body(velocity = vector(0,0,0), mass = 10, position = vector(0, 0, 0),identity = "sun")
y = Body(velocity = vector(0,.01,0), mass = 1, position = vector(-3,0,0), identity = "earth")
z = Body(velocity = vector(0,-.01,0), mass = 1, position = vector(3,0,0), identity = "mars")
