...
def PhysicalPopulate(CenterMass, RadialScaleLength, Numstars, VisibleMassNonSMBH, MinDist, Diameter):
 ...
 ##       AvgDensity = (MinDensity + MaxDensity) / 2
        MaxMass = ((TotMass * Max**2) / (2 *RadialScaleLength**2)) * (1 + Max**2/RadialScaleLength**2)**(-3/2)
        MinMass = ((TotMass * Min**2) / (2 *RadialScaleLength**2)) * (1 + Min**2/RadialScaleLength**2)**(-3/2)
        Mass = MaxMass-MinMass
        DivisionMass.append(Mass)
    for i in range(NumDivisions):
...
def VisualPopulate(ListOfBodies, Diameter):
    ...
    for i in range(len(ListOfBodies)):
        if i != 0:
            single = sphere (pos = ListOfBodies[i].position, radius = Diameter/2000 )
